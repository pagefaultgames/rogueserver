...
type ResetPWStore interface {
	RemoveSessionsFromUUID(uuid []byte) error
	UpdateAccountPassword(uuid []byte, newKey []byte, newSalt []byte) error
}

func ResetPW[T ResetPWStore](store T, username string, resetCode string, password string) error {
	if len(password) < 6 {
	...
	if err != nil {
		return fmt.Errorf("failed to fetch UUID: %s", err)
	}
